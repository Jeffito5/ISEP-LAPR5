...
encontrar_arm_ent(L):- findall(X,entrega(X,_,_,_,_,_),L).
encontrar_m_id_ent(LM,ID):-findall(M,entrega(_,_,M,ID,_,_),LM).

planeamento_entrega_distancia(LF,CC):-encontrar_arm_ent(LI), armazem_inicial(I),planeamento_entrega_distancia1(LI,I,LF1),remover_l(LF1,I,LF),transformar_lista_armazem([I|LF],LF2),custo(CC,LF2,eTruck01,LF).

planeamento_entrega_distancia_RLI(LI,LF):- armazem_inicial(I),planeamento_entrega_distancia1(LI,I,LF1),remover_l(LF1,I,LF).

transformar_lista_armazem([],[I]):-!,armazem_inicial(I).

transformar_lista_armazem([H|T],[H|L]):- armazem_inicial(I),I == H,transformar_lista_armazem(T,L).

transformar_lista_armazem([H|T],[A|L]):- entrega(H,_,_,A,_,_),transformar_lista_armazem(T,L).
...
planeamento_entrega_massa(LF,CC):-encontrar_arm_ent(LI), armazem_inicial(I),planeamento_entrega_massa1(LI,I,LF1), remover_l(LF1,I,LF),transformar_lista_armazem([I|LF1],LF2),custo(CC,LF2,eTruck01,LF).

planeamento_entrega_massa_RLI(LI,LF):- armazem_inicial(I),planeamento_entrega_massa1(LI,I,LF1),remover_l(LF1,I,LF).

planeamento_entrega_massa1([],_,[]):- !.
planeamento_entrega_massa1(LI,I,[P|LF]):- prox_entrega1(LI,P,I), remover_l(LI,P,LE),planeamento_entrega_massa1(LE,P,LF).


planeamento_entrega_massa_distancia(LF,CC):-encontrar_arm_ent(LI), armazem_inicial(I), planeamento_entrega_massa_distancia1(LI,I,LF1,0), remover_l(LF1,I,LF),transformar_lista_armazem([I|LF1],LF2),custo(CC,LF2,eTruck01,LF).

planeamento_entrega_massa_distancia_RLI(LI,LF):- armazem_inicial(I),planeamento_entrega_massa_distancia1(LI,I,LF1,0),remover_l(LF1,I,LF).

planeamento_entrega_massa_distancia1([],_,[],_):- !.
...
prox_entrega([],I,_):-armazem_inicial(I).
%um tempo exagerado
prox_entrega(L,P,I):- menor_tempo(L,P,I).

prox_entrega1([],I,_):-armazem_inicial(I).
...
menor_tempo1([X|L],Pf,_,I,T):- armazem_inicial(AI),I == AI, entrega(X,_,_,P,_,_),dadosCam_t_e_ta(_,I,P,T1,_,_), T1<T,menor_tempo1(L,Pf,X,I,T1).

menor_tempo1([X|L],Pf,_,I,T):- entrega(X,_,_,P,_,_),entrega(I,_,_,J,_,_),dadosCam_t_e_ta(_,J,P,T1,_,_), T1<T,menor_tempo1(L,Pf,X,I,T1).
...
maior_massa1([X|L],Pf,_,I,M):- entrega(X,_,LM,_,_,_) , M<LM, maior_massa1(L,Pf,X,I,LM).
...
%
total_peso_entrega([],[]):-!.
total_peso_entrega([H|LE],[P|LP]):- entrega(H,_,P,_,_,_),total_peso_entrega(LE,LP).

totalPeso(TPC,CID,TPE,LE):-total_peso_entrega(LE,TP1),dadosCamiao(CID,_,_,CM,PT,_),soma_peso([PT|TP1],TPE),TPC is PT+CM.
soma_peso([],0).
soma_peso([H|T],TP):-soma_peso(T,TP1),TP is TP1+H.


custo(CC,LF,CI,LE):- totalPeso(TPC,CI,TPE,LE),dadosCamiao(CI,EneTotal,_,_,_,_),armazem_inicial(AI),custo1(LF,TPC,TPE,CC,CI,AI,EneTotal).
%
%%dadosCam_t_e_ta(<nome_camiao>,<cidade_origem>,<cidade_destino>,<tempo>,<energia>,<tempo_adicional>).
...
% encontrar_o_melhor_caminho(C,E,TSol):-get_time(Ti),
% todos_os_caminhos(LLP), encontrar_o_melhor_caminho1(L,LLP),
% menorValor(L,C), encontrarPosicao(L,C,P), nth0(P,LLP,E), get_time(Tf),
% TSol is Tf-Ti.

%encontrar_o_melhor_caminho1([],[]).
% encontrar_o_melhor_caminho1([CC|L],[H|T]):-custo(CC,H,eTruck01),
% encontrar_o_melhor_caminho1(L,T).
